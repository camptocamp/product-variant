...
class TestArchiveAttributeValue(SavepointCase):
    ...
    def test_value_unlink(self):
        ...
        self.attribute_lines.write({"value_ids": [(6, 0, remaining_values.ids)]})
        # then unlink it
        self.red.unlink()
    ...
    def test_value_archive(self):
        template_values = self.env["product.template.attribute.value"].search(
            [("product_attribute_value_id", "=", self.red.id)]
        )
        products = template_values.ptav_product_variant_ids
        # should be red car and red pen
    ...
    def test_value_non_archiveable(self):
        pen_template_value = self.env["product.template.attribute.value"].search(
            [
                ("product_attribute_value_id", "=", self.red.id),
                ("attribute_line_id", "=", self.pen_attribute_line.id),
            ]
        )
